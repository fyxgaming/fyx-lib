...
class FyxPurse extends locking_purse_1.LockingPurse {
    ...
    async pay(rawtx, parents) {
        ...
        if (orderUnlockVout) {
            tx.txIns[0].setScript(bsv2_1.Script.fromBuffer(Buffer.from(new Array(1568).fill(0))));
            if (tx.txOuts[0].script.isSafeDataOut()) {
                tx.addTxIn(tx.txIns[0].txHashBuf, 0, bsv2_1.Script.fromBuffer(Buffer.from(new Array(25).fill(0))), 2 ** 32 - 1);
                const sourceRawTx = await this.blockchain.fetch(Buffer.from(tx.txIns[0].txHashBuf).reverse().toString('hex'));
                const sourceTx = bsv2_1.Tx.fromHex(sourceRawTx);
                parents.push({
