...
class AuthService {
    ...
    async generateKeyPair(id, password) {
        ...
        const salt = bsv2_1.Hash.sha256(buffer_1.Buffer.concat([buffer_1.Buffer.from(this.network), buffer_1.Buffer.from(id)]));
        const pass = bsv2_1.Hash.sha256(buffer_1.Buffer.from(password.normalize('NFKC')));
        const { hash } = await argon2.hash({ pass, salt, time: 100, mem: 1024, hashLen: 32 });
        if (!(new bsv2_1.Bn().fromBuffer(buffer_1.Buffer.from(hash)).lt(bsv2_1.Point.getN()))) {
            throw new Error('BigInteger is out of range of valid private keys');
        }
        const versionByteNum = this.network === 'main' ?
            bsv2_1.Constants.Mainnet.PrivKey.versionByteNum :
            bsv2_1.Constants.Testnet.PrivKey.versionByteNum;
        ...
        const privKey = bsv2_1.PrivKey.fromBuffer(keybuf);
        return bsv2_1.KeyPair.fromPrivKey(privKey);
    }
    async register(id, password, email, firstName = '', lastName = '') {
        id = id.toLowerCase().normalize('NFKC');
        const keyPair = await this.generateKeyPair(id, password);
        const bip39 = bsv2_1.Bip39.fromRandom();
        const bip32 = bsv2_1.Bip32.fromSeed(bip39.toSeed());
        const recoveryBuf = bsv2_1.Ecies.bitcoreEncrypt(bip39.toBuffer(), keyPair.pubKey, keyPair);
        const reg = {
        ...
        const msgHash = await bsv2_1.Hash.asyncSha256(msgBuf);
        const sig = bsv2_1.Ecdsa.sign(msgHash, keyPair);
    ...
    async recoverBip39(id, keyPair) {
        ...
        const recoveryBuf = bsv2_1.Ecies.bitcoreDecrypt(buffer_1.Buffer.from(recovery, 'base64'), keyPair.privKey);
        return bsv2_1.Bip39.fromBuffer(recoveryBuf);
    }
    async recover(id, keyPair) {
        const bip39 = await this.recoverBip39(id, keyPair);
        return bsv2_1.Bip32.fromSeed(bip39.toSeed());
    }
    ...
    async rekey(mnemonic, id, password) {
        const bip39 = bsv2_1.Bip39.fromString(mnemonic);
        const bip32 = bsv2_1.Bip32.fromSeed(bip39.toSeed());
        const keyPair = await this.generateKeyPair(id, password);
        const recoveryBuf = bsv2_1.Ecies.bitcoreEncrypt(bip39.toBuffer(), keyPair.pubKey, keyPair);
        ...
        }, id, bsv2_1.KeyPair.fromPrivKey(bip32.privKey)));
        return keyPair;
    }
